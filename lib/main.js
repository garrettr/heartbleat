  ...
  onStopRequest:
    ...
    if (components.isSuccessCode(status)) {
      ...
      if (passedCheck) {
        ...
        console.log("External service did no return all good, asking the user");
        // ask the user if they want to visit anyway
        let promptService = Cc["@mozilla.org/embedcomp/prompt-service;1"].getService(Ci.nsIPromptService);
        let checkbox = { value: false };
        let loadAnyway = promptService.confirmCheck(getMostRecentBrowserWindow(), "Heartbleed Warning", "This site is vulnerable to heartbleed. Are you sure you want to load it?", "Don't show again for this site.", checkbox);
        if (loadAnyway) {
